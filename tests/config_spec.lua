describe("albus-conflictius.config", function()
  ...
  it("returns defaults when setup() is never called", function()
    ...
    assert.is_true(resolved.celebrate)
  end)
  ...
  it("applies overrides", function()
    local resolved = config.setup({ banner = false, auto_stage = false, celebrate = false })
    assert.is_false(resolved.banner)
    assert.is_false(resolved.auto_stage)
    assert.is_false(resolved.celebrate)
  end)
  ...
  it("rejects a non-boolean celebrate value", function()
    assert.has_error(function()
      config.setup({ celebrate = "no" })
    end)
  end)
