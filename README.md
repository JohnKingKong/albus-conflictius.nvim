...
5. Hunks where both sides genuinely changed different things are left alone. Opening that file from the dashboard (`<CR>`) drops the cursor on the first remaining conflict, with the "ours"/"theirs" side of each hunk background-highlighted so they're visually distinct. Pressing `<CR>` accepts whichever side the cursor's line is on — same idea everywhere, single-pane or 3-pane. There are also explicit keymaps: `<leader>mo` accept ours, `<leader>mt` accept theirs, `<leader>mb` accept both, `<leader>mn`/`<leader>mp` jump to the next/previous conflict, `<leader>mw` run the wand right there on the buffer (no need to go back to the dashboard). No diff panes to decode by default — you're just looking at your real file. For the harder cases, `<leader>md` opens a 3-pane view — **ours** (left) | **result** (middle, your real file) | **theirs** (right) — where `<CR>` in either side pane accepts that side for the hunk under the cursor there. `q` closes the panes (non-destructive, `<leader>md` reopens them), or the whole view if they're already closed — with a confirmation prompt only if conflicts still remain.
...
7. When the *last* conflicted file in the repo gets resolved, the dashboard throws a little party — multicolor sparkle rows around the (static) wizard, then a laser beam / message / star-line banner underneath it, each row spanning the full width of the dashboard's own window so the message (picked once at random, e.g. "ALAKAZAM! ALL CONFLICTS VANISHED!", colored letter-by-letter) never gets clipped — before it closes. Press `<CR>`/`q`/`<Esc>` to skip it, or turn it off entirely with `celebrate = false`.
...
require("albus-conflictius").setup({
  ...
  celebrate = true,    -- play the fireworks/laser animation when the last conflict is resolved
})
```
...
**`lua/albus-conflictius/celebrate.lua`** — the colorful sparkle/laser/message animation played when the last conflict is resolved
