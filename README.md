...
7. When the *last* conflicted file in the repo gets resolved, the dashboard throws a little party — colorful fireworks and a sweeping laser beam around a whimsical banner (picked once at random, e.g. "ALAKAZAM! ALL CONFLICTS VANISHED!") play in the dashboard's own window, at its current size, before it closes. Press `<CR>`/`q`/`<Esc>` to skip it, or turn it off entirely with `celebrate = false`.
